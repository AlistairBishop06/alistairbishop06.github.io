...
const panels: Array<{ id: string; name: string; description: string; icon: IconName }> = [
  { id: 'display', name: 'Display', description: 'Change the desktop background and colour scheme.', icon: 'display' },
  { id: 'sounds', name: 'Sounds and Audio Devices', description: 'Adjust audio volume or turn sound off.', icon: 'sound' },
  { id: 'mouse', name: 'Mouse', description: 'Change pointer style and movement.', icon: 'mouse' },
  { id: 'users', name: 'User Accounts', description: 'Change the name shown around Portfolio XP.', icon: 'user' },
  { id: 'access', name: 'Accessibility Options', description: 'Adjust contrast and text size.', icon: 'accessibility' },
  { id: 'date', name: 'Date and Time', description: 'View your computer’s local date and time.', icon: 'date' },
  { id: 'programs', name: 'Add or Remove Programs', description: 'Inspect installed skills and technologies.', icon: 'programs' },
];
export function ControlPanel({ initialPanel }: { initialPanel?: string }) {
  const { open, settings, setSettings, play } = useSystem();
  ...
  return <div className="control-panel app-fill">
    ...
    {!panel ? <div className="control-grid">{panels.map(item => <button key={item.id} onClick={() => item.id === 'display' ? open('display') : setPanel(item.id)}><IconGlyph name={item.icon} size={42} /><span><b>{item.name}</b><small>{item.description}</small></span></button>)}</div> : <div className="control-detail">
      ...
      {panel === 'programs' && <div className="program-list">{skillGroups.flatMap(group => group.items.map(item => <div key={item.name}><IconGlyph name={group.icon} size={25} /><span><b>{item.name}</b><small>{group.name} · {item.level}</small></span><button onClick={() => open('skills', { skill: item.name })}>Details</button></div>))}</div>}
    </div>}
  </div>;
}
