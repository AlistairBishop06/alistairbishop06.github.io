...
export function InternetExplorer({ initialTarget, requestId }: { initialTarget?: BrowserTarget; requestId?: number }) {
  ...
  const menus = [
    ...
    { label: 'Favorites', items: [{ label: 'GitHub', action: () => window.open('https://github.com/alistairbishop06', '_blank', 'noopener,noreferrer') }] },
  ...
  return <div className="ie app-fill">
    ...
    <div className="ie-toolbar">
      ...
      <button onClick={() => go(home.url)}><IconGlyph name="home" size={26} /><span>Home</span></button><i />
    ...
    <div className="browser-viewport">
      {favoritesOpen && <aside className="favorites-pane"><header>Favorites <button onClick={() => setFavoritesOpen(false)}>×</button></header><button onClick={() => window.open('https://github.com/alistairbishop06', '_blank', 'noopener,noreferrer')}><IconGlyph name="favorites" size={17} /> Alistair on GitHub</button><button onClick={() => newTab(home)}><IconGlyph name="home" size={17} /> Google</button></aside>}
      {active.embed ? <div className="iframe-wrap">
