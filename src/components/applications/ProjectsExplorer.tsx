...
import { featuredProjectNames } from '../../data/featuredProjects';
...
export function ProjectsExplorer() {
  ...
  const visible = useMemo(() => sortRepositories(repos, includeForks).filter(repo => {
    const study = getCaseStudyForRepository(repo);
    const matchesMode = showAll || featuredProjectNames.has(repo.name.toLowerCase());
    const query = filter.toLowerCase();
    return matchesMode && (!query || repo.name.toLowerCase().includes(query) || repo.description?.toLowerCase().includes(query) || study.title.toLowerCase().includes(query) || study.tagline.toLowerCase().includes(query));
  }), [repos, includeForks, showAll, filter]);
  const openRepo = (repo: GitHubRepo) => {
    play('folder');
    const project = getCaseStudyForRepository(repo);
    open('project', { repo }, `${project.title} - Project Properties`);
  };
  return <ExplorerShell
    ...
    status={loading ? 'Contacting GitHub...' : `${showAll ? 'All projects · automatic case studies' : 'Featured case studies'} · ${source === 'live' ? 'Live GitHub data' : source === 'stale' ? 'Offline cache' : 'Local cache'}`}
  ...
  >
    ...
    >
      ...
      <span className="file-name">{study.title}</span>
      ...
      <span className="file-description">{study.tagline}{repo.topics?.length ? ` · ${repo.topics.slice(0, 3).join(', ')}` : ''}</span>
    </button>})}
  </ExplorerShell>;
}
