...
export function ProjectsExplorer() {
  ...
  return <ExplorerShell
    ...
    extraViewItems={[{ label: 'Show forks', checked: includeForks, action: () => setIncludeForks(value => !value) }]}
  >
    <div className="explorer-filter"><label>Search projects: <input value={filter} onChange={event => setFilter(event.target.value)} /></label></div>
    ...
    {!loading && visible.map(repo => <button
    ...
    >
      ...
      <span className="file-description">{repo.description || 'GitHub repository'}{repo.topics?.length ? ` · ${repo.topics.slice(0, 3).join(', ')}` : ''}</span>
    </button>)}
  </ExplorerShell>;
}
